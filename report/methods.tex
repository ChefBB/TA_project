...
The development of static models was simple and straight forward.
...
% Both Random Forest and SVM do not take \texttt{title} as input,
% working with the other attributes alone.
The preprocessing layers handle both \texttt{title} and
\texttt{lemmatized\_stanzas} through
Term Frequency-Inverse Document Frequency. The boolean attributes
...
training. The architectures were developed and tuned through empirical,
reiterated testing.\\

Both the Recurrent and One-Dimensional Convolutional Neural Network
have the same non recurrent architecture. As for static models,
Term Frequency-Inverse Document Frequency is used for preprocessing
\texttt{title} into topic analysis. Boolean attributes are converted into
integers, while \texttt{stanza\_number} is scaled. 
